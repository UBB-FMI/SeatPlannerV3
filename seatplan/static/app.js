...
import {confirmTranslated, currentLanguage, initializeTranslations, translate} from "./i18n.js";
...
class SeatplanApp
{
    ...
    async api(path, options = {})
    {
        ...
        if (response.ok === false)
        {
            ...
            const error = new Error(translate(String(detail)));
    ...
    async start()
    {
        ...
        if (location.hash.startsWith("#confirm="))
        {
            ...
            {
                this.toast(error.message || "This sign-in link could not be used. Request a new one.", true);
            }
        }
    }
        ...
        modal.querySelector("form").onsubmit = (event) =>
        {
            ...
            this.run(async () =>
            {
                ...
                try
                {
                    ...
                    modal.querySelector("#signin-message").innerHTML = `<p class="notice">${escape(result.message)} ${escape(translate("The link expires in 15 minutes."))}</p>`;
                }
                finally
                {
    ...
    async navigate(view)
    {
        if (this.editor?.dirty && confirmTranslated("Leave this editor and discard unsaved changes? Export or save the draft first to keep them.") === false)
        {
            return;
        }
    ...
    async refreshPublic()
    {
        ...
        document.querySelector("#event-date").textContent = `${snapshot.event.starts_at}${snapshot.event.status === "closed" ? ` · ${translate("Reservations closed")}` : ""}`;
        ...
        const remaining = snapshot.event.status === "open" ? this.remainingSeats() : 0;
        const available = [...this.selected].filter((id) => valid.has(id));
        this.selected = new Set(available.slice(0, remaining));
        if (this.selected.size < previous)
        {
            this.toast(available.length > remaining && snapshot.event.status === "open" ? "Your selection was reduced to the seats still available under this event’s per-email limit." : "Some selected seats are no longer available. Your selection was updated.", true);
        }
    ...
    togglePublicSeat(id)
    {
        ...
        if (seat === undefined || seat.status !== "free" || this.snapshot.event.status !== "open")
        {
            return;
        }
        ...
        {
            if (this.selected.size >= this.remainingSeats())
            {
                this.toast("This event's per-email seat limit has been reached.", true);
                return;
            }
    ...
    chosenSeats()
    {
        ...
        container.innerHTML = seats.length ? seats.map((seat) => `<button class="selected-chip" data-remove="${seat.id}" translate="no">${escape(seatName(seat))}<span aria-hidden="true">×</span></button>`).join("") : '<p class="muted">Click an available seat on the plan, or use the search below.</p>';
        ...
        this.root.querySelector("#booking-rule").textContent = event.max_per_user > 0 ? `${translate('Up to {0} seats per email for this event.', event.max_per_user)} ${this.me.email ? translate('{0} of {1} seats remain available to this email for this event.', remaining, event.max_per_user) : ''}` : "Multiple seats can be reserved with one email address.";
        const button = this.root.querySelector("#book-button");
        button.textContent = event.status !== "open" ? "Not reservable" : this.me.email ? `Reserve ${seats.length || "selected"} seat${seats.length === 1 ? "" : "s"}` : "Sign in to reserve";
        button.disabled = event.status !== "open" || (this.me.email && seats.length === 0);
    }

    remainingSeats()
    {
        const maximum = this.snapshot.event.max_per_user;
        if (maximum === 0)
        {
            return 100;
        }
        const alreadyReserved = this.me.email ? this.snapshot.seats.filter((seat) => seat.mine).length : 0;
        return Math.max(0, maximum - alreadyReserved);
    }
    ...
    seatResults()
    {
        ...
        container.innerHTML = seats.slice(0, 60).map((seat) => `<button class="seat-result ${this.selected.has(seat.id) ? "chosen" : ""}" data-seat="${seat.id}" ${this.snapshot.event.status !== "open" || seat.status !== "free" || (this.selected.has(seat.id) === false && this.selected.size >= this.remainingSeats()) ? "disabled" : ""}><span translate="no">${escape(seatName(seat))}</span><small>${seat.mine ? "Yours" : escape(seat.status)}</small></button>`).join("") + `<p class="small muted">${seats.length > 60 ? `Showing 60 of ${seats.length}. Refine your search.` : `${seats.length} matching seats.`}</p>`;
    ...
    async showMine()
    {
        ...
        this.root.innerHTML = `<div class="page-heading"><p class="eyebrow">YOUR PLACES</p><h1>My reservations</h1><p class="muted">${escape(this.me.email)}</p></div><div class="reservation-grid">${data.bookings.map((booking) => `<article class="reservation-card"><p class="eyebrow">${escape(booking.seats.length ? translate("{0} RESERVED", booking.seats.length) : translate("NO ACTIVE SEATS"))}</p><h2 translate="no">${escape(booking.title)}</h2><p class="muted" translate="no">${escape(booking.starts_at)}</p><div class="reservation-seats">${booking.seats.map((seat) => `<div><span>${escape(seatName(seat))}</span><button class="text-button danger-text" data-cancel="${booking.id}" data-seat="${seat.id}">Cancel seat</button></div>`).join("")}</div><p class="reference">${escape(translate("Reference: {0}", booking.id))}</p>${booking.seats.length ? `<button class="button secondary" data-cancel="${booking.id}">Cancel all these seats</button>` : ""}</article>`).join("") || '<div class="empty">You have no reservations yet.</div>'}</div>`;
        this.root.querySelectorAll("[data-cancel]").forEach((button) => button.onclick = () => this.run(async () =>
        {
            if (confirmTranslated("Cancel the selected reservation seats? They will immediately become available to others.") === false)
            {
                return;
            }
    ...
    async showAdmin(tab)
    {
        if (this.editor?.dirty && confirmTranslated("Discard unsaved map changes?") === false)
        {
            return;
        }
        this.editor?.destroy();
    ...
    {
        if (this.editor?.dirty && confirmTranslated("Discard unsaved map changes before refreshing the plan list?") === false)
        {
            return;
        }
        ...
        {
            if (this.editor?.dirty && confirmTranslated("Discard unsaved changes to this plan?") === false)
            {
    ...
    uploadDialog()
    {
        if (this.editor?.dirty && confirmTranslated("Save or export first to keep your edits. Continue and discard unsaved map changes?") === false)
        {
            return;
        }
    ...
    async showLive()
    {
        const data = await this.api("/api/events");
        const content = this.root.querySelector("#admin-content");
        content.innerHTML = `<div class="admin-toolbar"><label>Event<select id="live-event"><option value="">Select an event</option>${data.events.map((event) => `<option value="${event.id}">${escape(event.title)} · ${escape(translate(event.status))}</option>`).join("")}</select></label><button id="new-event" class="button">Create event</button><button id="edit-event" class="button secondary">Event settings</button><button id="refresh-live" class="button secondary">Refresh live view</button></div><div id="live-content"></div>`;
    ...
    async loadLive(id)
    {
        ...
        content.querySelector("#override-form").onsubmit = (event) =>
        {
            ...
            this.run(async () =>
            {
                ...
                if (confirmTranslated(translate("Set {0} seats to {1}? This affects {2} currently reserved seats. Attendees with email addresses will be notified.", count, translate(form.elements.action.value), displaced)) === false)
                {
                    return;
                }
    ...
    updateLiveSelection()
    {
        ...
        this.root.querySelector("#override-selection").innerHTML = `<strong>${escape(translate("{0} selected", seats.length))}</strong>` + seats.slice(0, 20).map((seat) => `<div>${escape(seatName(seat))}<small>${escape(translate(seat.allocation?.email || seat.status))}</small></div>`).join("") + (seats.length > 20 ? `<p>${escape(translate("+ {0} more", seats.length - 20))}</p>` : "");
        this.root.querySelector("#live-counts").innerHTML = ["free", "reserved", "blocked"].map((state) => `<div class="stat-line"><span>${escape(translate(state))}</span><strong>${snapshot.seats.filter((seat) => seat.status === state).length}</strong></div>`).join("");
    }
    ...
    liveRoster()
    {
        ...
        body.innerHTML = seats.map((seat) => `<tr><td><input type="checkbox" data-roster="${seat.id}" ${this.liveSelected.has(seat.id) ? "checked" : ""} aria-label="${escape(translate("Select {0}", seatName(seat)))}"></td><td>${escape(seatName(seat))}</td><td><span class="state-label ${seat.status}">${escape(translate(seat.status))}</span></td><td>${escape(seat.allocation?.email ?? "")}</td><td class="reference">${escape(seat.allocation?.booking_id ?? "")}</td><td>${escape(seat.allocation?.note ?? "")}</td></tr>`).join("");
    ...
    async showSystem()
    {
        ...
        const worker = data.worker_age_seconds === null ? translate("No worker heartbeat yet") : translate("Worker heartbeat: {0}s ago", data.worker_age_seconds);
        target.innerHTML = `<div class="system-summary"><div><h2>Delivery & operations</h2><p class="${data.worker_age_seconds === null || data.worker_age_seconds > 150 ? "danger-text" : "muted"}">${escape(worker)} · ${escape(translate("{0} mail backend", data.mail_backend))}</p><p class="muted">${Object.entries(data.mail_counts).map(([key, value]) => `${escape(translate(key))}: ${value}`).join(" · ") || "No messages queued yet."}</p></div><button id="refresh-system" class="button secondary">Refresh</button></div><section class="roster-panel"><h2>Email delivery</h2><div class="table-scroll"><table><thead><tr><th>Recipient</th><th>Subject</th><th>State</th><th>Attempts</th><th>Error</th><th></th></tr></thead><tbody>${data.messages.map((message) => `<tr><td>${escape(message.recipient)}</td><td>${escape(message.subject)}</td><td>${escape(translate(message.status))}</td><td>${message.attempts}</td><td>${escape(message.last_error)}</td><td>${["queued", "failed"].includes(message.status) ? `<button class="text-button" data-retry="${message.id}">Retry</button>` : ""}</td></tr>`).join("")}</tbody></table></div></section><section class="roster-panel"><h2>PDF jobs</h2><div class="table-scroll"><table><thead><tr><th>Job</th><th>Type</th><th>State</th><th>Error</th></tr></thead><tbody>${data.jobs.map((job) => `<tr><td class="reference">${job.id}</td><td>${job.kind}</td><td>${escape(translate(job.status))}</td><td>${escape(job.error)}</td></tr>`).join("")}</tbody></table></div></section><section class="roster-panel"><h2>Latest audit entries</h2><p class="small muted">Showing the latest 100 entries. The database retains the complete audit trail; the web interface cannot edit or delete it.</p><div class="table-scroll"><table><thead><tr><th>Time (local)</th><th>Actor</th><th>Action</th><th>Details</th></tr></thead><tbody>${data.audit.map((entry) => `<tr><td>${escape(new Date(entry.created * 1000).toLocaleString(currentLanguage()))}</td><td>${escape(entry.actor)}</td><td>${escape(entry.action)}</td><td class="audit-detail">${escape(entry.details)}</td></tr>`).join("")}</tbody></table></div></section>`;
...
initializeTranslations(app.base, (language) =>
{
    if (app.me?.email)
    {
        app.api("/api/me/language", {method: "POST", body: {language}}).catch((error) => app.toast(error.message, true));
    }
    if (app.view === "book" && app.snapshot)
    {
        app.root.querySelectorAll("#event-select option").forEach((option) =>
        {
            const event = app.eventOptions?.find((item) => item.id === option.value);
            if (event)
            {
                option.textContent = event.title + (event.status === "closed" ? ` · ${translate("closed")}` : "");
            }
        });
        app.run(() => app.refreshPublic());
    }
    else if (app.view === "mine")
    {
        app.run(() => app.showMine());
    }
    else if (app.view === "admin" && app.editor)
    {
        app.editor.refresh();
    }
    else if (app.view === "admin" && app.liveSnapshot && app.root.querySelector("#override-selection"))
    {
        app.map?.update(app.liveSnapshot.seats, [], app.liveSelected);
        app.updateLiveSelection();
        app.liveRoster();
    }
    else if (app.view === "admin" && app.root.querySelector("#refresh-system"))
    {
        app.run(() => app.showSystem());
    }
}).then(() => app.run(() => app.start()));
